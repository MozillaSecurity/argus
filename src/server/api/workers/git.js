...
tasks.process('clone', 4, function(task, done) {
  Repository.findById(task.data._id)
    .then(repository => {
      if (!repository) {
...
exports.startTaskClone = meta => {
  ...
  task
    ...
    .on('start', () => {
      ...
      Repository.findById({ _id: task.data._id })
        .then(repository => {
          if (!repository) {
    ...
    .on('failed', () => {
      ...
      Repository.findByIdAndRemove(task.data._id)
        .then(() => {
...
tasks.process('pull', 4, function(task, done) {
  Repository.findById(task.data._id)
    .then(repository => {
      ...
      GitManager.pull(repository.name)
        .then(() => {
          GitManager.log(repository.name)
            .then(entries => {
